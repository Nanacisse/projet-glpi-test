...
def start_analysis():
    """Fonction pour lancer l'analyse complète avec cache."""
    ...
    with st.spinner('Analyse en cours...'):
        ...
        # Vérifier si les résultats viennent du cache
        cache_used = hasattr(run_full_analysis, '_from_cache') and run_full_analysis._from_cache
        
        if cache_used:
            st.session_state['cache_used'] = True
            print("⚠ Résultats chargés depuis le cache, pas besoin de sauvegarder")
            save_success = True  # Pas besoin de sauvegarder en base
        else:
            # Sauvegarde dans la base de données seulement si pas de cache
        ...
        st.session_state['cluster_data'] = cluster_results
        st.session_state['pagination_offset'] = 0
        
    # Affichage du résultat final après l'analyse
    if st.session_state.get('cache_used', False):
        status_message_placeholder.success("✅ Résultats chargés depuis le cache (données inchangées)")
    elif save_success:
